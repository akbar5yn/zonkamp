...
<div class="container">
	...
	<div class="judul">
		...
		<div class="form">
			...
			<form action="<?= base_url('profile/proses_edit_video') ?>" method="POST" enctype="multipart/form-data">
				<input type="hidden" name="id_content" value="<?= $getDataVideo['id_content'] ?>">
				<label for="fname">Judul</label>
				<input type="text" value="<?= $getDataVideo['judul'] ?>" id="fname" name="judul"
					placeholder="Masukan Judul.." required>
				<label for="subject">Konten</label>
				<div class="kotak-upload">
					<label id="form" for="" class="video-up">
				...
				<div id="form" class="result-video">
					...
					<div class="bungkus-video" id="bungkus" data-getvideo="<?= $getDataVideo['content_link'] ?>">
						<i class="fas fa-3x fa-play-circle"></i>
						<video src="<?= $getDataVideo['content_link'] ?>" class="result" controls id="outputVideo">
						</video>
					</div>
				</div>
				<textarea id="subject" name="desc_content" placeholder="Write something.."
					style="height:200px"><?= $getDataVideo['desc_content'] ?></textarea>
				<label for="kategori">Pilih Kategori</label>
				<select id="kategori" name="id_kategori">
					<!-- <option value="<?= $getDataVideo['id_kategori'] ?>"></option> -->
